...
from models import EfficientAT_Lite, MobileNetV1Audio
...
# MobileNetV1 (朋友的模型)
mobilenet_model_path = resolve_model_path("best_model.pth")
...
# =========================================================
# xyr 的 AST 七分类模型（HuggingFace transformers）
# 类别: alarm, baby_cry, car_horn, Doorbell, glass_shatter, gun_shot, negative
# 输入: 16kHz 单声道，固定 10 秒
# =========================================================
ast_path = resolve_model_path("ast_best_model.pt")
ast_label_map_path = resolve_model_path("ast_label_map.json")
if not ast_path.exists():
    raise FileNotFoundError(f"未找到 xyr 的 AST 模型文件: {ast_path}")
if not ast_label_map_path.exists():
    raise FileNotFoundError(f"未找到 xyr 的 AST label_map: {ast_label_map_path}")

print(f"正在加载 AST (xyr): {ast_path}")
ast_label_data = json.loads(ast_label_map_path.read_text(encoding="utf-8"))
ast_label_names = [
    ast_label_data["id_to_label"][str(i)]
    for i in range(len(ast_label_data["id_to_label"]))
]
ast_config = ASTConfig(
    num_labels=len(ast_label_names),
    id2label={i: label for i, label in enumerate(ast_label_names)},
    label2id={label: i for i, label in enumerate(ast_label_names)},
)
ast_model = ASTForAudioClassification(ast_config)
ast_ckpt = torch.load(str(ast_path), map_location=device, weights_only=False)
ast_state = (
    ast_ckpt["model_state_dict"]
    if isinstance(ast_ckpt, dict) and "model_state_dict" in ast_ckpt
    else ast_ckpt
)
ast_model.load_state_dict(ast_state)
ast_model.to(device).eval()
print(f"xyr 的 AST 模型已加载: {ast_path}  类别={ast_label_names}")

# AST 专用特征提取器（参数与 release_ast_7class/predict.py 完全一致）
...
CONF_THRESHOLDS = {
    ...
    "ast": 0.60,              # xyr 的 AST 七分类，softmax 单标签
}
...
async def predict(
    ...
    支持的 model_choice:
      ...
      - "ast"         : xyr 的 AST 七分类（HuggingFace transformers）
    """
    import tempfile
    ...
    try:
        ...
        # ==================== xyr 的 AST 七分类模型分支 ====================
        # AST 训练时输入固定 10 秒，这里不能用 select_best_3s_window 截断，
        # 需要保留完整音频（前端在 AST 模式下会录制 10 秒）。
        if model_choice == "ast":
            target_10s = 16000 * 10
            if len(y) < target_10s:
                y_ast = np.pad(y, (0, target_10s - len(y)), mode='constant')
            else:
                y_ast = y[:target_10s]

            features = ast_feature_extractor(
                y_ast, sampling_rate=16000, return_tensors="pt"
            )
            with torch.no_grad():
                logits = ast_model(
            ...
            top_label = ast_label_names[top_idx]   # 例如 "Doorbell" 或 "negative"
            top_prob = float(probs[top_idx])
            effective_conf = (
                float(conf_threshold) if conf_threshold is not None
                else CONF_THRESHOLDS["ast"]
            )
            effective_conf = max(0.0, min(1.0, effective_conf))

            results = []
            # 过滤 negative 类（无事件/背景音），其余类别小写化以复用前端 trans 字典
            if top_label.lower() != "negative" and top_prob > effective_conf:
                results.append({"class": top_label.lower(), "prob": top_prob})
...
if __name__ == "__main__":
    ...
    print("  • ast          - AST 七分类 (xyr 的模型, 需 10 秒输入)")
    print("")
    print("实时优化已启用：")
    ...
    # 默认绑定 0.0.0.0 是为了方便从其他设备访问测试
    # 但本地开发时请务必用 localhost 打开页面
